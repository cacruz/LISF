...
!BOP
! !ROUTINE: noahmp401_wrf_f2t
! \label{noahmp401_wrf_f2t}
!
! !REVISION HISTORY:
...
!
! !INTERFACE:
subroutine noahmp401_wrf_f2t(n)
! !USES:      
  use ESMF
...
!  lisimpdataname(21) = 'Sea Ice Mask'
  ...
  call ESMF_StateGet(LIS_FORC_State(n),trim(LIS_FORC_Xice%varname(1)),&
       xiceField,rc=status)
  call LIS_verify(status,'noah401_f2t: error getting xice')
  ...
  !call LIS_verify(status,'noah401_f2t: error retrieving xice')


  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     tid = LIS_surface(n,1)%tile(t)%tile_id 
     noahmp401_struc(n)%noahmp401(t)%tair=tmp(tid)
     noahmp401_struc(n)%noahmp401(t)%qair=q2(tid)
     noahmp401_struc(n)%noahmp401(t)%swdown=swd(tid)
     noahmp401_struc(n)%noahmp401(t)%lwdown=lwd(tid)
     noahmp401_struc(n)%noahmp401(t)%wind_e=uwind(tid)
     noahmp401_struc(n)%noahmp401(t)%wind_n=vwind(tid)
     noahmp401_struc(n)%noahmp401(t)%psurf=psurf(tid)
     !noahmp401_struc(n)%noahmp401(t)%xice=xice(tid)
     if(pcp(t).ne.LIS_rc%udef) then 
        noahmp401_struc(n)%noahmp401(t)%prcp=pcp(tid)
     else
        noahmp401_struc(n)%noahmp401(t)%prcp=0.0
     endif
     noahmp401_struc(n)%noahmp401(t)%snowf=0.0

     if(ch(tid).ne.LIS_rc%udef) then 
        noahmp401_struc(n)%noahmp401(t)%ch=ch(tid)
     endif

     !if(chs2(tid).ne.LIS_rc%udef) then 
     !   noahmp401_struc(n)%noahmp401(t)%chs2=chs2(tid)
     !endif

     !if(cqs2(tid).ne.LIS_rc%udef) then 
     !   noahmp401_struc(n)%noahmp401(t)%cqs2=cqs2(tid)
     !endif

     !if(q2sat(tid).ne.LIS_rc%udef) then 
     !   noahmp401_struc(n)%noahmp401(t)%q2sat=q2sat(tid)
     !endif

     if(emiss(tid).ne.LIS_rc%udef) then 
        noahmp401_struc(n)%noahmp401(t)%emiss=emiss(tid)
     endif

     !if(zval(tid).ne.LIS_rc%udef) then 
     !   noahmp401_struc(n)%noahmp401(t)%z=zval(tid)
     !endif
     noahmp401_struc(n)%forc_count = 1
  enddo

end subroutine noahmp401_wrf_f2t
