...
!  lisimpdataname(21) = 'Sea Ice Mask'
  ...
  call ESMF_StateGet(LIS_FORC_State(n),trim(LIS_FORC_Xice%varname(1)),&
       xiceField,rc=status)
  call LIS_verify(status,'noah36_f2t: error getting xice')
  ...
  call LIS_verify(status,'noah36_f2t: error retrieving xice')
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     ...
     noahmp36_struc(n)%noahmp36(t)%xice=xice(tid) ! not used?!
     if(pcp(t).ne.LIS_rc%udef) then 
        noahmp36_struc(n)%noahmp36(t)%prcp=pcp(tid)
     else
        noahmp36_struc(n)%noahmp36(t)%prcp=0.0
     endif
     noahmp36_struc(n)%noahmp36(t)%snowf=0.0

     if(ch(tid).ne.LIS_rc%udef) then 
        noahmp36_struc(n)%noahmp36(t)%ch=ch(tid)
     endif

     if(chs2(tid).ne.LIS_rc%udef) then 
        noahmp36_struc(n)%noahmp36(t)%chs2=chs2(tid) ! not used?!
     endif

     if(cqs2(tid).ne.LIS_rc%udef) then 
        noahmp36_struc(n)%noahmp36(t)%cqs2=cqs2(tid) ! not used?!
     endif

     if(q2sat(tid).ne.LIS_rc%udef) then 
        noahmp36_struc(n)%noahmp36(t)%q2sat=q2sat(tid) ! not used?!
     endif

     if(emiss(tid).ne.LIS_rc%udef) then 
        noahmp36_struc(n)%noahmp36(t)%emissi=emiss(tid)
     endif

     if(zval(tid).ne.LIS_rc%udef) then 
        noahmp36_struc(n)%noahmp36(t)%z=zval(tid) ! not used?!
     endif
